...
def runsim(roundnum=15000,CHECK=False,
    ...
    else raise ValueError("unknown result type"+result_type)
    ...
    while x <= roundnum: 
        ...
        mut = numpy.random.lognormal(0,mut_sd,3) 
        ...
        if x%rpt_freq==0:
            if result_type=="timeseries":
                ...
                results[x//rpt_freq,1,,] = parAlst
                results[x//rpt_freq,2,,] = parBlst
                results[x//rpt_freq,3,,] = colourlst
            x+=1
