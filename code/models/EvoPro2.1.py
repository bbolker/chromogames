...
def runsim(roundnum=15000,CHECK=False,
    ...
    results = numpy.array((roundnum // rpt_freq,8))
    ...
    if a_range_init[0]==0 and a_range_init[1]==0:
        parAlist = numpy.zero(w,w)
    else:
        parAlst = numpy.random.uniform(a_range[0],a_range[1],size=(w,w))
    if b_range_init[0]==0 and b_range_init[1]==0:
        parBlist = numpy.zero(w,w)
    else:
        parBlst = numpy.random.uniform(b_range[0],b_range[1],size=(w,w))
