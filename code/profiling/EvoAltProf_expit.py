		...
		count1=0
                ...
                for i in range(len(D1)):
                        ...
                        for j in range(d,len(D1)-d):
                                I1=D1[i]%w
                                I2=D2[j]%w
                                coop_ctr = scipy.special.expit(parAlst[m][n])
				coop_nbr = scipy.special.expit(parAlst[I1][I2])
	...
	if ntype=='neu':
		...
		for i in range(len(D1)):
			...
			for j in range(d,len(D1)-d):
				...
				Z2=D2[j]%w
				if switch=='off':
                                        coop_ctr = scipy.special.expit(parAlst[dc1][dc2])
                                        coop_nbr = scipy.special.expit(parAlst[Z1][Z2])
					t_ctr=coop_ctr*coop_nbr*R + coop_ctr*(1-coop_nbr)*S + (1-coop_ctr)*coop_nbr*T + (1-coop_ctr)*(1-coop_nbr)*P 
