...
sys.stderr.write("Testing if the number of cells remains constant.\n") 

## Testing if the number of cells remains constant.  
...
def runsim(roundnum=10000,CHECK=False,
	...
	if ((len(parAlst) == ww) and (len(parBlst) == ww) and (len(colourlst) == ww)): 
		print(True, "number of cells unchanged") 
	else: 
		print("error; unexpected grid size") 
		sys.exit()	
        x+=1
...
def main():
        ...
        x = runsim(a_range_init=(al,au),w=ww,R=RR,P=PP,T=TT,S=SS,roundnum=rn,seed=sd[0],rpt_freq=freq,CHECK=False,result_type='timeseries',mut_sd=(m1,m2,m3),ntype=nntype,mut_type=mtp,nsize=2,switch='off') 
	numpy.save('storagecheck.npy',x) 
        print('test complete')
